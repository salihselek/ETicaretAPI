...
namespace ETicaretAPI.Persistence.Services
{
    internal class AuthorizationEndpointService : IAuthorizationEndpointService
    {
        ...
        public async Task AssignRoleEndpointAsync(string[] roles, string menu, string code, Type type)
        {
            ...
            if (_menu is null)
                _menu = await createMenu(menu);

            Endpoint endpoint = await _endpointReadRepository.Table.Include(e => e.Menu).Include(e => e.Roles)
                .FirstOrDefaultAsync(e => e.Menu.Id == _menu.Id && e.Code == code);
            if (endpoint is null)
            {
                Application.DTOs.Configuration.Action? action = _applicationService.GetAuthorizeDefinationEndpoints(type)
                    .FirstOrDefault(e => e.Name == menu)?.Actions.FirstOrDefault(a => a.Code == code);
                endpoint = await createEndpoint(action, _menu);
            }
        ...
        public async Task<List<string>> GetRolesByEndpointAsync(string code, string menu)
        {
        ...
        private async Task<Endpoint> createEndpoint(Application.DTOs.Configuration.Action action, Menu menu)
        {
        ...
        private async Task<Menu> createMenu(string menu)
        {
            Menu _menu = new()
            {
                Id = Guid.NewGuid(),
